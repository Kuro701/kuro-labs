...
const link=(url,label,primary=false)=>url?`<a class="kl-action${primary?' kl-primary':''}" href="${e(url)}"${/^https?:/.test(url)?' target="_blank" rel="noopener noreferrer"':''}>${e(label)} ↗</a>`:'';
...
for(const p of data.products){
 ...
 const body=`<div class="kl-wrap kl-detail-page">${back('shop',p.name)}<article><div class="kl-detail-layout">${media}<div><span class="kl-tag">${e(p.specs[0][1])}</span><h1>${e(p.name)}</h1><p>${e(p.short)}</p><div class="kl-price">${e(p.price)}</div><div class="kl-actions">${link(p.buy,'Buy on Gumroad',true)}${link(p.buy2,'Buy on Jinxxy',true)}</div><p class="kl-buy-note">One-time purchase · Checkout opens on Gumroad.</p></div></div><div class="kl-project-body"><h2>About this asset</h2><p style="margin-top:18px">${e(p.long)}</p>${specs(p.specs)}</div></article></div>`;
